...
def generate_tasks(task_date: str, db: Session = Depends(get_db), _=Depends(require_user)):
    """
    Генерує завдання на випічку. «Замовлено» рахується від ЗАМОВЛЕНЬ (клієнти + магазин),
    а не від накладних — щоб у Випічці окремо бачити Замовлено / Корекції (зміни в накладних) /
    Спечено / Відхилення (узгоджено зі звітом випічки).
