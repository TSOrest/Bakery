...
- **Друк:** генерація PDF (reportlab або weasyprint)
- **Розгортання:** повністю локально, localhost
...
### Фаза 1 — MVP (ПОЧИНАЄМО ТУТ)
- [ ] Структура проекту + `install.sh`
- [ ] SQLite схема (`database/schema.sql`)
- [ ] FastAPI app + database session
- [ ] Models + Schemas для: products, categories, clients, routes, prices
- [ ] CRUD endpoints для довідників
- [ ] Логіка замовлень: POST/GET/PUT + копіювання з дати
- [ ] Сервіс `get_price()` з пріоритетами
- [ ] Завдання на випічку + внесення результату
- [ ] Накладні: генерація + автонумерація YYYYMMDD-NNN
- [ ] React каркас: навігація по вкладках + вкладка Замовлення
- [ ] Базові тести (pytest)

### Фаза 2 — Повний цикл
- [ ] Stale-логіка: несвіжий товар по всьому ланцюжку
- [ ] Скасування рейсу
- [ ] Магазин: звірка, несвіжий, ІНШЕ
- [ ] Авторизація і ролі
- [ ] Друк PDF (накладні, завдання пекарям)
...
## Важливі деталі (узгоджені в Chat)
...
- Windows-середовище (основна ОС замовника)
