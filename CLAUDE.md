...
- [x] Системний трей `tray.py` (pystray):
  ...
  - ⚠ Баг (виправлено): `_fetch_latest_tag()` гасив будь-яку помилку запиту до
    GitHub без жодного слідy в логах — на машині клієнта запит падав
    (типово: Python/`ssl` не довіряє тому самому сертифікату, що й браузер,
    напр. корпоративний антивірус/проксі з власним TLS-сертифікатом,
    встановленим у довіру Windows, але не в довіру Python), і оператор при
    ручній перевірці бачив хибне "Встановлена остання версія: X" замість
    правди "перевірку не вдалось виконати". Додано `truststore` (перемикає
    Python `ssl` на системну довіру Windows — той самий стандарт, що й у
    браузера) + лог помилки (`[UPDATE]` у денному лог-файлі) + чесне
    повідомлення при провалі перевірки.
