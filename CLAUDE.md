...
**Наступні кроки** (Крок 2+): підключення `require_perm()` до самих
роутерів 5 адмін-груп і Фінансів + hard-rule захист від ескалації прав
через Users CRUD (роль з делегованим `admin_system.edit` не повинна
мати змогу зробити когось адміном чи редагувати вже-адмінський акаунт);
нова CRUD-матриця в `RolePermissionsTab.tsx`; дії-рівневе блокування
кнопок у ~14 admin-компонентах.
