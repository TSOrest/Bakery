...
- [x] Системний трей `tray.py` (pystray):
  ...
  - balloon-сповіщення: зміна стану сервера, нова версія, початок оновлення, зміна стану інтернету (state-based, без rate-limit)
  - `_poll_internet` thread (кожні 30 сек): моніторить з'єднання, нотифікує при втраті/відновленні
  - PowerShell toast fallback (`scripts/notify.ps1`) якщо `icon.notify()` недоступний
