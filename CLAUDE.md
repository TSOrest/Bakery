...
## Фікс: клієнти без замовлень приховані у списку Маршрутів

`RoutesPage.tsx` — рядки клієнтів у стані `no_activity` ("немає замовлень",
приглушені сірим) типово ПРИХОВАНІ у списку "Всі клієнти"/рейс/"Внутрішні",
а не показані завжди: на реальних маршрутах їх переважна більшість (напр.
39 клієнтів рейсу, 9 із замовленням — 30 порожніх рядків без жодної дії
займали весь екран прокрутки). Новий стан `showInactive` (default `false`,
скидається в `false` при кожній зміні активної KPI-картки маршруту) —
`shownClients` фільтрує `sortedClients` за станом, `inactiveClients` рахує
скільки приховано. Внизу списку — кнопка-перемикач
"▾ Показати без замовлень (N)" / "▴ Сховати без замовлень (N)"
(`.showInactiveBtn`), видима лише коли `inactiveClients.length > 0`.
Сортування (`sortedClients`) і решта логіки (checkbox-масові дії,
`needsInvoiceClients` тощо) — без змін, бо вони й так ніколи не включали
`no_activity`-клієнтів.

## Фіча: скасування помилково внесеного переміщення (список/пайок)

Оператор хотів мати змогу виправити помилково внесене списання/пайок без
переходу в "Корекція / переміщення": іконка "✕" прямо біля виноски
"передано → .../отримано від ...", яка коректно скасовує саме це
переміщення (повертає кількість назад).

- **`POST /invoices/transfers/{transfer_id}/cancel`** (`backend/routers/invoices.py`,
  `cancel_transfer`) — знаходить `InvoiceTransfer`, повертає `t.qty` назад у
  рядок джерела (той самий пошук за `product_id`+`line_kind`, що й при
  створенні переміщення — `next()`, перший знайдений рядок), знімає `t.qty`
  з рядка цілі (видаляє рядок цілі повністю, якщо кількість впала до ~0),
  перераховує `total_sum` і `recompute_invoice_finance` обох накладних,
  видаляє сам запис `InvoiceTransfer`.
  - **Дозволено лише доки ОБИДВІ накладні (джерело і ціль) ще `draft`/`sent`**
    (`_TRANSFER_CANCEL_STATUSES`) — навмисно вужче за звичайну панель
    "Корекція / переміщення" (яка працює і для `accepted`): швидке
    скасування розраховане на "щойно помилково натиснув", не на розбір
    давно завершених операцій. 400, якщо будь-яка зі сторін уже `accepted`/
    `cancelled`.
  - Захист від неконсистентного стану: якщо рядок цілі вже змінений іншою
    дією (напр. ручне редагування через `PUT /invoices/{id}/lines`) до
    кількості, меншої за `t.qty` — 400 замість мовчазного некоректного
    відняття ("Кількість у цільовій накладній вже змінена іншою дією").
  - Працює однаково для self-target переміщень ("↺ продано як звичайний" —
    src.id == tgt.id, обмінний рядок → платний рядок того ж клієнта).
- **Frontend** (`RoutesPage.tsx`, `InvoiceDetailPanel`): кнопка "✕"
  (`.transferCancelBtn`) поруч із КОЖНОЮ виноскою переміщення (основна
  таблиця І секція «Обмін» — той самий генеричний ендпоінт для обох),
  видима лише коли `invoice.status === 'draft' || 'sent'`
  (`canCancelTransfers`, дзеркалить бекенд-обмеження). Підтвердження через
  `useConfirm()` (danger) з текстом виробу/кількості/напрямку. Помилки з
  бекенду (напр. "вже змінено іншою дією") показуються оператору дослівно
  через новий локальний `extractApiErrorDetail()` (той самий хелпер, що
  вже є в `IssuesWidget.tsx` — парсить `detail` з тіла помилки `api/client.ts`).

Тести: `tests/test_invoice_transfer_cancel.py` (успішне скасування з
відновленням кількості й видаленням спорожнілого рядка цілі; відмова коли
джерело вже `accepted`; відмова коли рядок цілі вже зменшений іншою дією).

## Фікс: розміщення QR-коду бота на друкованій накладній

QR-код (`invoice_bot_qr_enabled`, див. "Реліз v1.3.x — Telegram-бот, Розділ E")
раніше друкувався в самому низу накладної, під підписами "Прийняв"/
"Відпускає" — за фідбеком користувача перенесено у верхню секцію, поруч із
мета-полями ("Від кого"/"Кому"/"Через"/"Довіреність №"), праворуч: текст
"Замовляйте через бот" тепер під QR-кодом (стовпчиком), а не збоку.
Виправлено дзеркально в обох незалежно написаних рендерах (той самий
принцип, що й решта фіксів навколо друку — див. "Реліз v1.3.4"):
- **`render_invoice_block`** (HTML для браузера, `BASE_CSS`): мета-таблиця й
  `_bot_qr_html()` обгорнуті у новий `.meta-row` (`display:flex`) — QR
  займає фіксовані 18mm праворуч, мета-таблиця розтягується на решту
  (`flex:1`). `.bot-qr` перебудовано з горизонтального (`align-items:center`)
  на вертикальний (`flex-direction:column`) — картинка згори, підпис під нею,
  по центру. Коли QR вимкнено — `.meta-row` містить лише таблицю, порожній
  рядок не займає місця (без QR вигляд ідентичний попередньому).
- **`render_invoice_pdf_bytes`** (ReportLab PDF для Telegram): QR-комірка
  (`Image` + `Paragraph` у вкладеній `Table`) тепер рахується ПЕРЕД
  мета-таблицею, щоб визначити скільки ширини (`meta_w`) лишається їй; обидві
  об'єднуються в один рядок (`meta_row`, вкладена `Table`) замість окремого
  блоку після підписів.

## Фікс: Довідка — спливаюче вікно замість повної сторінки, згорнуті секції, версія

За фідбеком — кнопка ❓ у шапці раніше переводила на окрему сторінку
`/help` (повністю ховала поточний екран, включно з даними, які оператор
хотів звірити з довідкою). Тепер відкриває **спливаюче вікно**
(`frontend/src/components/HelpPopup.tsx`, новий) поверх поточної
сторінки:
- `position:fixed`, розмір під вміст (560px, `maxWidth:92vw,
  maxHeight:82vh`, власний внутрішній скрол), БЕЗ темного фону-оверлея —
  решта сторінки лишається видимою й клікабельною.
  Початкова позиція — під кнопкою ❓ (верхній правий кут, той самий
  принцип, що вже є в `NotificationBell.tsx`'s випадаючому списку,
  `top:54, right:16`).
- **Перетягується** за шапку вікна (`onMouseDown` на заголовку →
  `mousemove`/`mouseup` на `document`, той самий патерн, що вже є в
  `RoutesPage.tsx`'s `startDrag` для розсувної панелі) — можна відсунути
  вбік і бачити дані під ним. Закривається ЛИШЕ кнопкою "✕" — навмисно
  без закриття по кліку поза вікном (на відміну від `NotificationBell`'s
  dropdown): сенс переміщення вікна — клікати по даних під ним, не
  втрачаючи довідку.
- `HelpPopup` не дублює вміст — рендерить існуючий `<HelpPage />` у
  скрол-контейнері; сам компонент і маршрут `/help` (`App.tsx`) лишились
  без змін, деталі HelpPage тепер просто крутяться у двох контекстах
  (окрема сторінка за прямим посиланням і спливаюче вікно з шапки).
- `Layout.tsx`: кнопка ❓ — з `NavLink` на звичайний `<button>` (тепер
  перемикає локальний стан `helpOpen`, не навігує); стилізовано так само
  (`.tab`/`.active` з `Layout.module.css`), з ручним `background:'none'`
  для не-активного стану (компенсує дефолтний фон `<button>`, якого не
  було в `<a>`; при `helpOpen` — `background` не форситься inline, щоб не
  перекривати `.active` клас).

**Секції довідки згорнуті за замовчуванням**: `Section` (`HelpPage.tsx`)
мала `useState(true)` — увесь довідник одразу розгорнутим суцільним
текстом. Змінено на `useState(false)` — оператор бачить компактний список
заголовків розділів, розгортає потрібний.

**Версія програми в довідці**: футер показував
`import.meta.env.VITE_APP_VERSION` — змінну оточення, яка НІКОЛИ не
встановлювалась (0 звернень у всьому репозиторії, ані в `vite.config.ts`,
ані в build-скриптах) — тому завжди показувала заглушку "—". Виправлено:
`GET /settings/server-info` (вже існуючий, легкий, `require_user`
ендпоінт — раніше повертав лише `local_ip`/`port` для POS URL,
`ShopPage.tsx`) розширено полем `app_version` (`_read_version()`,
той самий хелпер, що вже читає `backend/routers/backup.py` для бекапів;
імпортовано напряму — cross-router імпорти вже усталений патерн у
`backend/routers/*.py`, напр. `require_perm` з `auth.py` майже всюди,
`_compute_section1_data` з `print_views.py` в `reports.py`). `HelpPage.tsx`
підвантажує версію через `useEffect`+`api.get`, замінивши мертву
build-time змінну на живі дані з бекенду.

Заразом виправлено текст підказки "💬 Якщо щось не працює..." — раніше
казав "у правому нижньому куті" (стара позиція плаваючої кнопки
Звернень, до попереднього фіксу цієї ж сесії, що переніс її в шапку) —
тепер "у шапці (поруч із дзвоником сповіщень)".

## Фікс: плаваюча кнопка «Звернення» перенесена в заголовок

За фідбеком — плаваюча кнопка 💬 (`IssuesWidget.tsx`, `.fab`,
`position:fixed; bottom:24px; right:24px`) перекривала дані в правому
нижньому куті широких таблиць (напр. таблиця звірки магазину). Перенесено
з окремого floating-button у рядок заголовка (`Layout.tsx`,
`headerRight`) — між дзвоником сповіщень 🔔 і посиланням «Довідка» ❓,
той самий inline-стиль кнопки без фону/рамки (`background:none,
border:none, fontSize:1.15rem, padding:0.3rem 0.4rem`), що вже
використовує `NotificationBell.tsx` для 🔔. `.fab` CSS-клас видалено
(більше не використовується). Саме модальне вікно (`.backdrop`/`.modal`,
`position:fixed; inset:0`) поведінково не залежить від того, де в DOM
змонтований компонент — `position:fixed` коректно центрується відносно
вьюпорта незалежно від вкладеності в `.headerRow` (жоден з предків
(`.root`/`.header`/`.headerRow`) не встановлює `transform`/`filter`,
що зламало б containing block для fixed-елементів). Як і раніше — лише
на сторінках під `Layout` (НЕ на `/pos`, окремий кіоск-шелл, куди
`IssuesWidget` ніколи й не монтувався).

## Фікс: низ таблиці звірки (read-only перегляд минулих звірок) обрізався

⚠ Баг (виявлено користувачем при перевірці попереднього фіксу групування,
непов'язаний з ним напряму — існував і раніше): у вкладці Магазин, при
прокручуванні детального перегляду минулої звірки (клік на дату в
«Звірки (календар)») до самого низу, останні рядки і фінальний рядок
«Всього» (`<tfoot>`) не показувались — обрізались, незалежно від
прокручування.

Причина — класична помилка каскаду висот у вкладеному flexbox:
`ShopTabContent` (`frontend/src/pages/ShopPage.tsx`) повертав ПРОСТИЙ
Fragment (`<>...</>`) із двома блочними дітьми: блок «📦 Залишки
магазину» (природна висота, `maxHeight:220`) і обгортку `<ReconciliationCalendar>`
зі стилем `height: '100%'`. Оскільки Fragment нічого не рендерить у DOM,
обидва div-и опинялись звичайними блочними (не flex-) дітьми батьківського
контейнера (`ShopPage`, `flex:1; minHeight:0; overflow:hidden`, БЕЗ
`display:flex`). У звичайному block-flow `height:100%` рахується від
висоти контейнера — тож другий блок отримував 100% висоти контейнера
**ДОДАТКОВО** до висоти першого блоку над ним, сумарний вміст перевищував
контейнер, і `overflow:hidden` обрізав нижню частину (саме нижню — блоки
йдуть згори вниз) — рівно стільки, скільки займав блок «Залишки» вище.
Внутрішній `overflowY:auto` самої таблиці прокручував лише СВІЙ власний
вміст коректно, але сам контейнер таблиці був наперед обрізаний ззовні,
тож прокрутка не могла дістатись до реально відсутньої (обрізаної)
частини.

Виправлено: `ShopTabContent` тепер повертає справжній flex-column div
(`height:'100%', display:'flex', flexDirection:'column', overflow:'hidden'`)
замість Fragment; блок «Залишки» отримав `flexShrink:0` (фіксована
природна висота, не розтягується); обгортка `<ReconciliationCalendar>`
змінена з `height:'100%'` на `flex:1, minHeight:0` (займає РІВНО залишок
висоти після «Залишків», без подвійного рахунку). Той самий патерн
(`flex:1, minHeight:0` для елемента, що ділить простір із сусідом, замість
`height:100%`, який коректний лише для єдиної дитини) — уже правильно
використаний у самому `ReconciliationCalendar` (`sectionBox, flex:1,
minHeight:0`) і в батьківському `ShopPage` (`flex:1, minHeight:0` навколо
`<ShopTabContent>`) — тільки в одній конкретній точці каскаду (Fragment)
цей принцип був порушений.

## Фікс: групування виробів за категорією у таблиці звірки магазину

За фідбеком операторів — таблиця звірки магазину (`ReconciliationTable`,
`frontend/src/pages/ShopPage.tsx`, живий екран внесення залишків при
активній звірці) показувала всі вироби одним суцільним списком, без
розділення на Хліб/Булка/Інше — незручно шукати потрібний рядок.
Виправлено групуванням за `category_id` виробу з підзаголовком-роздільником
(рядок на всю ширину таблиці, `colSpan={9}`) перед кожною групою; групи
сортуються за порядком категорій у довіднику (`sort_order`, той самий
принцип "індекс у масиві `categories`", що вже використовується в
read-only перегляді минулих звірок нижче в тому ж файлі — `selectedRec`-
таблиця, `sortedGroups` там був зразком для цього фіксу), "Без категорії"
завжди останньою. Усередині групи рядки сортуються за назвою виробу.

**Технічна деталь**: `ReconciliationTable` раніше отримувала лише
`productName()` (функція для відображення назви), без самих `products`/
`categories` з `category_id`/`sort_order` — додано як нові пропси
(`ReconciliationModal` тепер довантажує `category_id` в `/products/` і
окремо `/categories`, дзеркалячи вже наявний паттерн з батьківського
`ShopPage`). **Нумерація `idx` для `inputRefs`/Tab-Enter-навігації
(`StreamInput`) перерахована на основі НОВОГО згрупованого порядку**
(`idxByLineId`, Map `line.id → позиція в сортованому списку`), а не
вихідного порядку `rec.lines` з бекенду — інакше клавіатурна навігація
переходила б по рядках у порядку, що не збігається з тим, що оператор
бачить на екрані.

Модалку одноразового **«◈ Початковий залишок»** (`OpeningRecModal`) —
свідомо НЕ чіпав: за попереднім рішенням користувача (див. пам'ять
`project_shop_opening_stock_decision.md`) ця кнопка навмисно рідко
використовується (один раз на магазин, при першому запуску обліку), не
щоденний робочий інструмент, і користувач не згадував її в цьому фідбеку.

## Фікс: версія програми в /help Telegram-бота для персоналу/власників

`STAFF_HELP` (`backend/services/telegram_bot.py`) був статичним рядком —
власник, перевіряючи через бота (`/help`/`/допомога`), не міг дізнатись
яка версія застосунку встановлена (актуально після автооновлення —
власник не завжди має доступ до трею на сервері). Замінено на функцію
`_staff_help_text()`, яка формує підказку динамічно з новим хелпером
`_read_version()` — читає кореневий файл `VERSION` (той самий формат/файл,
що вже читає `backend/routers/backup.py::_read_version()` для
`/settings/server-info`; продубльовано, а не імпортовано звідти, щоб не
тягнути router-модуль у сервіс бота). Заголовок підказки тепер
"Команди бота Пекарня (версія vX.Y.Z):". Версія читається на кожен виклик
`/help` (не кешується) — оновлюється лише через реальне оновлення
застосунку, яке й так перезапускає процес.

Тест: `tests/test_low_findings_section5.py::test_staff_help_mentions_app_version`.

## Фікс: крупніший шрифт друкованої накладної (читабельність для людей похилого віку)

За фідбеком — серед клієнтів і персоналу пекарні є люди похилого віку, яким
друкована накладна (`render_invoice_block`/`BASE_CSS`,
`backend/routers/print_views.py`; 2 копії поряд на A4 landscape,
`/print/invoice/{id}` і `/print/invoices`) важко читається через дрібний
шрифт. Збільшено шрифт по всіх елементах накладної (9–9.5pt → 10–11.5pt для
основного тексту, 14pt → 17pt для заголовка, 13pt → 16pt для підсумкової
суми), ширини числових колонок таблиці виробів пропорційно розширені
(Кільк. 38→44px, Од. 32→38px, Ціна 54→68px, Сума 60→**88px**, Обм. 26→32px —
в ОБОХ місцях шаблону, основна таблиця й секція «Обмін», + `obmin_th`), і
трохи стиснуті декоративні відступи (`.page-pair` padding/gap, `.inv-block`
padding) щоб компенсувати частину забраної ширини. Колонка «Назва» — і без
того без фіксованої ширини — тепер переноситься на 2 рядки частіше
(`.n { overflow-wrap: break-word; }` додано як захист від задовгого слова
без пробілів); користувач це підтвердив як прийнятний компроміс.

⚠ Проміжний баг (виявлений і виправлений під час перевірки, ДО коміту):
перша спроба лишила колонку «Сума» на 70px — при більшому жирному шрифті
підсумок категорії з чотиризначною сумою (напр. «1 216,20») сам переносився
на 2 рядки в клітинці («1» окремо зверху) — потворно і non-obvious без
живого рендеру. Виявлено ЛИШЕ завдяки візуальній перевірці через
headless-друк (`msedge.exe --headless --print-to-pdf` на реальний HTML
відповіді, той самий підхід що вже є в `render_html_to_pdf_bytes`) —
текстові асерти в тестах цього б не спіймали. Виправлено розширенням
колонки до 88px.

**`@page` margin (5mm) і `body`/`.baking-*` з `BASE_CSS` НЕ чіпались** —
той самий CSS-блок підключається ще до 4 інших друкованих форм
(`print_baking`, `print_baking_report`, `daily_report` через спільний
`{BASE_CSS}`), і `@page`/`body` там нічим не перекриваються (на відміну
від `print_baking_report`/`daily_report`, які додають свій `@page`
override, `print_baking` — ні). Усі змінені селектори (`.page-pair`,
`.inv-block`, `.inv-top`, `.city`, `.inv-date`, `.inv-title`, `.meta-tbl`,
`.lines-tbl`, `.total-line`/`.total-box`, `.sigs`, `.bot-qr`) підтверджено
використовуються ВИКЛЮЧНО в `render_invoice_block` — перевірено, що
збільшення шрифту й компенсаційне стиснення відступів ніяк не зачіпає
друк випічки/звіту випічки/денного звіту (перевірено живими запитами —
`body`/`@page` там лишились 10pt/5mm як і раніше). Окремий PDF-рендер для
Telegram-бота (`render_invoice_pdf_bytes`, ReportLab) — свідомо НЕ
чіпався (узгоджено з користувачем): це інший носій (телефон, є зум), не
папір.
