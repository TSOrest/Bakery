...
function Find-ExistingInstall {
    # Шукає існуючу установку за завданням BakeryApp у планувальнику задач
    try {
        $task = Get-ScheduledTask -TaskName $APP_TASK -ErrorAction SilentlyContinue
        if (-not $task) { return $null }
        $action = $task.Actions | Select-Object -First 1
        # Варіант 1: WorkingDirectory завдання
        if ($action.WorkingDirectory -and (Test-Path "$($action.WorkingDirectory)\backend")) {
            return $action.WorkingDirectory
        }
        # Варіант 2: витягуємо шлях з аргументу -File "...\scripts\run-server.ps1"
        if ($action.Arguments -match '-File\s+"?([^"]+)\\scripts\\run-server\.ps1"?') {
            $dir = Split-Path $Matches[1] -Parent
            if (Test-Path "$dir\backend") { return $dir }
        }
    } catch {}
    return $null
}
...
$dbBackupPath = $null
if ($isReinstall -and (Test-Path "$InstallDir\bakery.db")) {
    $dbBackupPath = "$env:TEMP\bakery_reinstall_$PID.db"
    Copy-Item "$InstallDir\bakery.db" $dbBackupPath -Force
    Write-Info 'База даних збережена для відновлення'
}
...
# Відновлюємо БД якщо git clone видалив папку
if ($dbBackupPath -and (Test-Path $dbBackupPath)) {
    Copy-Item $dbBackupPath "$InstallDir\bakery.db" -Force
    Remove-Item $dbBackupPath -Force -ErrorAction SilentlyContinue
    Write-Info 'Базу даних відновлено'
}
